...
abstract class Controller
{
    ...
    public function filterAction($name, $arguments)
    {
            ...
            if ($this->before() === true) { // run another code before calling the actual method
                if ($_SERVER['REQUEST_METHOD'] == 'POST') {
                    ...
                    if (!static::checkCsrfToken()) {
                        View::throwAppError("Invalid CSRF token - Reload the page");
                    }
                }
        ...
        } elseif (method_exists($this, $name . 'NonFilteredAction')) {
    ...
    protected function checkApiFailure($api_response, $generic_error = null)
    {
        /** if no api response received */
        if (!$api_response) {
            if ($generic_error) {
                throw new AppError($generic_error);
            } else {
                throw new AppError("Unable to get the data");
            }
            return;
        }

        /** if api responsed with success `false` value, then hanlde */
        if (isset($api_response["success"]) && $api_response["success"] === false) {
            $messages = Utils::getEmptyArrayIfKeyNotExists($api_response, "messages");
            if ($messages) {
                throw new AppError(Utils::arrayMessageToString($messages));
            } elseif ($generic_error) {
                throw new AppError($generic_error);
            } else {
                throw new AppError("Unable to get the data");
            }
